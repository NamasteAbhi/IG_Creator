...
def generate_useragent():
    with open("UserAgent.txt", "r")as file:
        ...
        user = a.split(",")
    return f'Instagram 261.0.0.21.111 Android ({user[7]}/{user[6]}; {user[5]}dpi; {user[4]}; {user[0]}; {user[1]}; {user[2]}; {user[3]}; en_US; 436384447)'
...
if 'account_created":true' in response.text:
    ...
    fun = f'{usname}:{toka}'
    # requests.get("https://api.telegram.org/bot"+str('5320803158:AAFsJnD79PQPNDxbkHa1KnpBw37E6uUF6vc')+"/sendMessage?chat_id="+str('1995995358')+"&text="+(fun))
    ...
    aa = response.headers['ig-set-authorization'].split('Bearer IGT:2:')[1]
    mystr_encoded = base64.b64decode(aa).decode('utf-8')
    hakoer = json.loads(mystr_encoded)
    sessid = hakoer['sessionid']
    ...
    aa = cl.account_change_picture(DP())
    print('Profile Uploaded')
    cl.account_edit(biography=spintax.spin(bio_spintax),
                    full_name=username)
    print('Bio Added')
